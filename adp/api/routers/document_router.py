...
async def read_document(document_id: str, db: Session = Depends(get_db)):
...
@limiter.limit("60/minute")
async def create_document(request: Request, payload: DocumentCreateRequest, db: Session = Depends(get_db)):
...
async def update_doc_status(payload: DocumentUpdateStatusRequest, db: Session = Depends(get_db)):
