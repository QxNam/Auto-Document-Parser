...
def docling_process_cpu(path: str) -> str:
    ...
    with open(path, "rb") as f:
        docs = DocumentStream(name=os.path.basename(path), stream=io.BytesIO(f.read()))
        conv_result = doc_converter.convert(docs)

    result = conv_result.document.export_to_markdown(page_break_placeholder="<PAGE_BREAK>")
...
if __name__ == "__main__":
    ...
    pdf_path = "./tests/services/cccd.jpg"
